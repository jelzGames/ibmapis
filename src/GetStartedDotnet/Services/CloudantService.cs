...
namespace GetStartedDotnet.Services
{
    public class CloudantService : ICloudantService
    {
        ...
        public async Task<dynamic> CreateAsync(UserModel item)
        {
        ...
        {
            var _client = _factory.CreateClient("cloudant");
            var response = await _client.GetAsync(_client.BaseAddress + _dbName + "/" + id);
        ...
        public async Task<dynamic> Update(UserModel item, Guid id)
        {
            var _client = _factory.CreateClient("cloudant");

            string jsonInString = JsonConvert.SerializeObject(item);

            var response = await _client.PutAsync(_client.BaseAddress + _dbName + "/" + id + "?rev=" + item._rev, new StringContent(jsonInString, Encoding.UTF8, "application/json"));
        ...
        public async Task<dynamic> Delete(Guid id, string rev)
        {
            ...
            var response = await _client.DeleteAsync(_client.BaseAddress + _dbName + "/" + id + "?rev=" + rev);
